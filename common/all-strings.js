...
const queries = {
  select: {
    ...
    USER_STATUS: (trigram) =>
      `SELECT actif FROM ${user_role_table} WHERE user = '${trigram}';`,

    USER_ROLE_BY_TRIGRAM: (trigram) =>
      `SELECT role_slug, actif FROM ${user_role_table} WHERE user = '${trigram}' AND actif = '1';`,
    ...
    ALL_DIRECTION: `SELECT direction FROM ${user_table};`,

    ALL_MANAGER: `SELECT n_1 FROM ${user_table};`,

    ALL_RULE_SLUG: `SELECT slug FROM ${role_table};`,

    ALL_PERMISSION: `SELECT slug FROM ${permission_table};`,
  ...
  insert: {
    ...
    NEW_USER: (user) =>
      `INSERT INTO ${user_table} (trigramme, matricule, nom, prenom, email, direction, fonction, n_1) VALUES 
            ('${user.trigram.toUpperCase()}', ${user.matricule}, '${
        user.name
      }', '${user.firstName}', '${user.email}', '${user.direction}', '${
        user.job
      }', '${user.chief}');`,
